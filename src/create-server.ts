...
import { z } from "zod";
import { textResult, workatoFetch } from "./workato-api.js";

export function createWorkatoMcpServer(): McpServer {
  ...
  server.tool(
    ...
    async (params) => {
      ...
      return textResult(await workatoFetch("/recipes", query));
    }
  );
  ...
  server.tool(
    ...
    async ({ recipe_id }) => textResult(await workatoFetch(`/recipes/${recipe_id}`))
  );
  ...
  server.tool(
    ...
    async (params) => {
      ...
      if (params.page) query.page = params.page;
      if (params.per_page) query.per_page = params.per_page;
      return textResult(await workatoFetch("/folders", query));
    }
  );
  ...
  server.tool(
    ...
    {},
    async () => textResult(await workatoFetch("/folders", { parent_id: "0" }))
  );
  ...
  server.tool(
    ...
    "List recent jobs (execution history) for a specific recipe. Useful for checking failures.",
    {
      recipe_id: z.string().describe("The recipe ID"),
      status: z
        .enum(["succeeded", "failed", "pending", "aborted"])
        .optional()
    ...
    async (params) => {
      ...
      if (params.page) query.page = params.page;
      if (params.per_page) query.per_page = params.per_page;
      return textResult(await workatoFetch(`/recipes/${params.recipe_id}/jobs`, query));
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      const q: Record<string, string> = { adapter_names_any: params.query };
      if (params.running) q.running = params.running;
      if (params.page) q.page = params.page;
      if (params.per_page) q.per_page = params.per_page;
      return textResult(await workatoFetch("/recipes", q));
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      ...
      if (params.page) query.page = params.page;
      if (params.per_page) query.per_page = params.per_page;
      return textResult(await workatoFetch("/connections", query));
    }
  );
